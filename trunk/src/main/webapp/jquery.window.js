 ...
 * Customized Button JSON Array Sample:
	var myButtons = [
		...
		image: "../../../sources/img/facebook.gif",    // optional, the image url of button icon(16x16 pixels)
		callback:                     // required, the callback function while click it
			function(btn, wnd) {
				wnd.getContainer().find("#demo_text").text("Share to facebook!");
				wnd.getContainer().find("#demo_logo").attr("src", "../../../sources/img/facebook_300x100.png");
			}
		},
		// twitter button
		{
		...
		image: "../../../sources/img/twitter.png",
		callback:
			function(btn, wnd) {
				wnd.getContainer().find("#demo_text").text("Share to twitter!");
				wnd.getContainer().find("#demo_logo").attr("src", "../../../sources/img/twitter_300x100.jpg");
			}
		}
	];
 */
...
$.Window = (function()  {
	...
	function constructor(caller, options) {
		...
		var options = $.extend({
			...
			width: 725,                   // [number:400] window width
			height: 470,                  // [number:300] window height
			minWidth: 400,                // [number:200] the minimum width, if -1 means no checking
